...
| `BATON_USER_ID_HMAC_KEY` | _(unset)_ | Per-tenant secret keying the end-user `user_id` hash. The raw principal is hashed at the edge, so no sink ever sees it. Unset means `user_id` is skipped and events still emit; it is additive analytics, never a consent gate. **Generate it with `openssl rand -hex 32`** — the input space is emails and user ids, which is small and guessable, so a memorable key is not a weaker pseudonym, it is none: anyone holding the events can dictionary-attack the column back to raw identities. |
