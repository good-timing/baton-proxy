...
def test_the_renamed_hmac_env_var_is_never_read_and_warned_about_only_when_it_matters(
    ...
    # The CLAIM, not the prose: a warning naming a field the envelope retired
    # sends the reader grepping for something that cannot appear. Rewording
    # stays free; reintroducing the dead name reds. ⚠ Lowercase is load-bearing
    # — `BATON_PRINCIPAL_ID_HMAC_KEY` is upper case, so the env var name
    # asserted above cannot satisfy this by accident.
    assert "principal_id" not in text
    assert ("`principal`" in text) is warns
